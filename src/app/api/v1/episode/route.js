...
export async function GET(request, { params }) {
    ...
    if (!uuid && !id && !slug) {
        return NextResponse.json({ error: 'must send only one param: uuid/id/slug' }, { status: 400 })
    }
    ...
    if (uuid) {
        ...
        if (dbEpisodeInfo.numericValue) {
            ...
            return NextResponse.json({ error: 'something wrong with param: id' }, { status: 400 })
        }
    } else if (slug) {
