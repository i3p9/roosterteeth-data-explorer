...
import {
	bytesToReadableSize,
	copyToClipboard,
	getArchivedPercentageAndDataBySeasonId,
...
const BrowseSeasonContainer = (props) => {
	...
	const getPct = async (showUuid, season_id) => {
		const result = await getArchivedPercentageAndDataBySeasonId(
			showUuid,
			season_id
		);
			...
			<li className='text-color-primary'>
				<div className='bg-color-primary p-2 rounded flex flex-col items-start mb-4'>
					<div className='w-full flex justify-between'>
						<Link
							href={`/download/${showSlug}/season/${season?.attributes.slug}`}
						>
							<span className='font-bold text-xl text-color-primary hover:underline decoration-1 underline-offset-4'>
								Season: {season?.attributes.number} -{" "}
								{season?.attributes?.title}
								<span className='text-sm font-normal italic text-red-300'></span>{" "}
								<span className='text-sm font-normal italic text-purple-300'>
									{season?.attributes?.has_bonus_feature
										? "[Bonus Content]"
										: ""}
								</span>
							</span>
						...
						<a
							target='_blank'
							href={`https://roosterteeth.com/series/${season?.attributes.show_slug}?season=${season?.attributes.number}`}
						>
					...
					<div className='my-2 flex gap-2'>
						...
						/>
						<TotalSizeBadge
							size={bytesToReadableSize(totalSizeInByte)}
						/>
						{season?.attributes?.episodes_available?.sponsor && (
							<FirstBadgeBig />
						)}
					</div>
					<div>
