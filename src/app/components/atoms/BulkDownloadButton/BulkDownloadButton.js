...
export default function BulkDownloadButton({
		...
		data.forEach((episode) => {
			if (episode.archive.files && episode.archive.id) {
				...
				for (const file of episode.archive.files) {
					...
					if (
						file.file_ext.toLowerCase() === "jpg" ||
						file.file_ext.toLowerCase() === "png" ||
						file.file_ext.toLowerCase() === "jpeg" ||
						file.file_ext.toLowerCase() === "gif"
					) {
	...
	return (
		<div className={`${disabled && "make-disable"}`}>
