import { getShowInfo, copyToClipboard, makeTitle, formatSecondsToDuration } from '@/data/utils/utils';
                ...
                <div>
                    ...
                    >
                        <div className="flex flex-col">
                            ...
                            <h1 className="font-bold pb-0.5 text-color-primary line-clamp-2">{episode?.attributes.title}</h1>
                            <div className="flex items-center">
                                ...
                                />
                                <span className="ml-2 text-sm text-color-primary">{makeTitle(episode?.attributes?.channel_slug)} • {episode?.attributes.original_air_date.split('T')[0]}</span>
                            </div>
                        </div>
                    </Link >
                </div>
            ...
            <div className='hidden md:block'>
                <div className="grid md:grid-cols-3 items-start max-w-6xl mx-auto px-4 gap-2 lg:gap-4 m-4">
                    ...
                    <div className="grid gap-1 lg:gap-2 col-span-2">
                        ...
                        <p className="text-sm font-medium leading-none text-color-secondary">From: {showInfo ? showInfo[0]?.attributes?.title : 'Show Data not available'}</p>
                        <div className="flex items-center">
                            <img
                                alt={`logo of channel ${episode?.attributes.channel_slug}`}
                                className="w-6 h-6 rounded-full"
                        ...
                        <Popup
                            trigger={
                                <div className='hidden lg:block'>
                                    <p className="text-sm leading-snug text-color-secondary line-clamp-1">{episode?.attributes?.description}</p>
                                </div>
                            }
                            modal
                        >
                        ...
                        <div className='flex gap-4'>
                            <p className='text-xs font-medium text-color-faded'>RT Link: {' '}
