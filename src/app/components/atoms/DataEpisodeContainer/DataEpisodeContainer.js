				...
				<div className=' p-2 bg-color-primary rounded flex items-start'>
					...
					<div className='p-1 w-8/12 md:w-10/12'>
						<div>
							...
							<div className='hidden md:block text-color-secondary text-xs md:text-sm'>
								Air date:{" "}
								{episode?.attributes.original_air_date?.split("T")[0]}{" "}
								| Runtime:{" "}
								{formatSecondToRunTime(episode?.attributes.length)}
							</div>
							<div className='block md:hidden text-color-secondary text-xs'>
								{episode?.attributes.original_air_date?.split("T")[0]}
							</div>
							<p className='md:line-clamp-1 hidden text-sm text-color-faded '>
								{episode?.attributes?.description
									? episode?.attributes?.description
									: "N/A"}
							</p>
							<div className='flex gap-8 mt-1'>
								<p className='hidden md:block text-xs font-medium text-color-faded'>
									RoosterTeeth Link:{" "}
									<Link
